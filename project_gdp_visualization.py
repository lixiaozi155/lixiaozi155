...
def reconcile_countries_by_name(plot_countries, gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
    ...
    guozai={}
    guobuzai=set()
    for i,j in plot_countries.items():
        if j in gdp_countries:
            guozai[i] = plot_countries[i]
        else:
            guobuzai.add(i)
    return(guozai,guobuzai)
    # 编码，结束后将pass删除
    # 不要忘记返回结果
    ...
    for a,b in jieguo1[0].items():
        ...
        for i in range(gdpinfo["min_year"],gdpinfo["max_year"]+1):
            ...
            if zidian[b][str(i)] =='':
                ...
                c = c + 1
        if zidian[b][year]!='':
            guoGDP[a]=math.log10(float(zidian[b][year]))
            set2.add(a)
        if a == 0:
            set1.add(a)
        else:
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    worldmap_chart.title = '全球大国GDP分布图'
    worldmap_chart.add(year,dict1)
    worldmap_chart.add("该国缺少GDP数据",list(set1))
    worldmap_chart.add('该国该年无数据',list(set2))
...
def test_render_world_map(year):  #测试函数
    ...
    render_world_map(gdpinfo, pygal_countries, year, "isp_gdp_world_name_1988.svg")
